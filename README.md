...
[Drools](https://kie.apache.org/) has been around since 2001 and there numerous customers who have build entire production applications around rules and the associated engines `embedded` into large Java applications.  In some situations this architeture was followed because for many years that's all Drools provided.  Around 2018 the concept of a remote engine runtime was introduced, known as the `KIE Server`.  The KIE Server essentially delegated execution of rules and processes to a J2EE web application that required an appliation server to drive it's most powerful features.  Some customers migrated to the KIE Server approach while others remain in the embedded architecture.  IBM BAMOE is an enterprise build not only on Drools and jBPM, but also the new cloud-native framework known as `Kogito`.  With IBM BAMOE v9, which is based soley on Kogito's build of Drools and jBPM, customers now have a new option for engine deployment via standalone Quarkus or Spring Boot applications, or via containers.  For customers who are still unable to move to a non-embedded architecture, IBM BAMOE v9 offers embedded mode, so that customers can continue to run thier legacy applications with minimal upgrades, while still benefiting from the support subscription IBM BAMOE offers its enterprise customers.  The following sample architeture represents what a typical application architecture would look like using IBM BAMOE v9 in embedded mode:
