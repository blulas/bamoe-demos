# IBM Business Automation Manager Open Editions (IBM BAMOE) Demos - Classic Decision Model Service
This module contains an example `Classic Decisoin Model Service`, using the class implementation model vs using Kogito, packaged as a micro-service JAR, and using the `IBM BAMOE Quarkus runtime`.
